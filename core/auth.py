...
SESSION_EXPIRY = int(os.getenv("SESSION_EXPIRY", 3600))  # 1 hora por padrão
LOGIN_ATTEMPTS_LIMIT = int(os.getenv("LOGIN_ATTEMPTS_LIMIT", 5))

# Caminho para o arquivo de usuários (em produção, usar banco de dados)
USERS_FILE = Path(__file__).parent.parent / "data" / "users.json"

# Garantir que o diretório data existe
USERS_FILE.parent.mkdir(exist_ok=True)

# Inicializar o arquivo de usuários se não existir
...
def login_user(user):
    ...
    st.session_state.login_attempts = 0 # Resetar tentativas após login bem-sucedido
    st.session_state.last_activity = time.time()
